...
namespace FikaAmazonAPI.Services
{
  public class ReportService : RequestService
  {
    ...
    public async Task<string> GetReportFileAsync(string filename, string reportDocumentId, bool isRestrictedReport = false, CancellationToken cancellationToken = default)
    {
      var reportDocument = await GetReportDocumentAsync(reportDocumentId, isRestrictedReport, cancellationToken);

      return await GetFileAsync(filename, reportDocument, cancellationToken);
    }
    ...
    private async Task<string> GetFileAsync(string fileName, ReportDocument reportDocument, CancellationToken cancellationToken = default)
    {
    ...
    public async Task<string> CreateReportAndDownloadFileAsync(ReportTypes reportType, DateTime? dataStartTime = null, DateTime? dataEndTime = null, ReportOptions reportOptions = null, bool isRestrictedReport = false, List<MarketPlace> marketplaces = null, int millisecondsDelay = 500, CancellationToken cancellationToken = default)
    {
      ...
      while (string.IsNullOrEmpty(ReportDocumentId))
      {
        ...
        {
          var filename = $"{reportData.DataStartTime.Value.ToShortDateString().Replace("/", "")} - {reportData.DataEndTime.Value.ToShortDateString().Replace("/", "")}";

          filePath = await GetReportFileAsync(filename, reportData.ReportDocumentId, isRestrictedReport, cancellationToken);
          break;
        }
    ...
    public async Task<IList<string>> DownloadExistingReportAndDownloadFileAsync(string storageLocation, ReportTypes reportTypes, DateTime? createdSince = null, DateTime? createdUntil = null, CancellationToken cancellationToken = default)
    {
      ...
      if (reports != null)
      {
        foreach (var reportData in reports)
        {
          if (!string.IsNullOrEmpty(reportData.ReportDocumentId))
          {
            string folder = $"{Directory.GetParent(storageLocation).FullName}\\{reportData.DataStartTime.Value.ToString("yyyy")}";
            var filename = $"{folder}\\{reportData.DataStartTime.Value.ToShortDateString().Replace("/", "")} - {reportData.DataEndTime.Value.ToShortDateString().Replace("/", "")}";
            var filePath = await GetReportFileAsync(filename, reportData.ReportDocumentId, cancellationToken: cancellationToken);
